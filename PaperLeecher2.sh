...
# Default values
USE_PROXY=0
PROXY_URL="http://your-proxy-server:port" # Replace with your proxy server details
...
show_help() {
    ...
    echo "  -p, --proxy Use a proxy server for requests."
    echo "  -m, --max_pages Set the maximum number of pages to process (default is 5)."
    ...
    case $1 in
        ...
        -m|--max_pages) 
            if [[ -n "$2" && "$2" =~ ^[0-9]+$ ]]; then
                ...
                echo "Error: '--max_pages' requires a numeric argument."
                exit 1
            fi
            ;;
        *) SEARCH_TERM="$1" ;;
...
download_pdfs_from_page() {
    ...
    local curl_opts="-A \"$USER_AGENT\" -s"
